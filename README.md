...
<p align="center">
  <img src="https://img.shields.io/badge/sig-0.5.4-2f6fe0?style=flat-square" alt="Sig 0.5.4" />
...
- **Sig `0.5.4` or newer.** sls uses the native `sig_build` build runner; the
  legacy `std.Build` graph does not compile under the in-process runner. Sig
  `0.5.4` is required for the bare-metal `aarch64-sb0` image (it completes
  freestanding linking of the soft-float `compiler_rt` and the memory
  intrinsics the image pulls in); hosted targets build with any `0.5.x`.
