//! Layer 0 — Sig declaration scanner for document symbols.
//!
//! A small, allocator-free lexer that extracts declarations (`const`, `var`,
//! `fn`) from Sig source. It is intentionally lightweight: it recognises the
//! declaration-introducing keywords, captures the following identifier, and
//! classifies the symbol kind by a cheap look at the initializer (`fn` →
//! Function, `= struct` → Struct, `= enum` → Enum, `= union` → Interface,
//! otherwise Constant/Variable). Byte offsets are returned so callers can map
//! them to LSP positions.
//!
//! This is not a full parser — sls grows a real one for semantic features — but
//! it is accurate for the top-level and container-level declarations that drive
//! the document-symbol outline, and it never allocates.

const std = @import("std");

/// LSP SymbolKind numeric codes (subset used by sls).
...
/// Read the identifier starting at `i` (must be an ident start). Returns the
/// slice and the index just past it.
...
/// Whether `word` sits at `src[i..]` as a whole token (not part of a longer
/// identifier).
...
/// Classify a declaration by peeking at the bytes after the `=` for the given
/// identifier. `after_name` is the offset just past the declaration's name.
fn classifyValue(src: []const u8, after_name: usize) SymbolKind {
    var i = after_name;
    // Skip to '=' (bail at ';' or newline-terminated simple decls).
...
/// declarations (struct fields expressed as decls, methods) are reported with a
/// depth > 0.
pub fn scan(src: []const u8, out: *Scan) void {
    ...
    while (i < src.len) {
        ...
        // Skip string literals so keywords inside them are ignored.
        if (c == '"') {
            i += 1;
...
test "scans top level declarations with kinds" {
    ...
    // std, Point, main, counter, Color (fields x are not decls)
...
test "tracks nesting depth for methods" {
    ...
    try std.testing.expectEqual(@as(u16, 0), items[0].depth); // S
    try std.testing.expectEqual(@as(u16, 1), items[1].depth); // method
